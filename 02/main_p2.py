...
y_pos = 0 # vertical
for d in data:
    if d.movement == "forward": # forward = increase x position by d.position and increase y position by d.position * aim
        x_pos += d.position
        y_pos += d.position * aim
    elif d.movement == "down": # down = increase aim
        aim += d.position
    elif d.movement == "up": # up =  decrease aim
        aim -= d.position

    print(f"{d.movement} - {d.position} [{x_pos}, {y_pos}][{aim}]")
